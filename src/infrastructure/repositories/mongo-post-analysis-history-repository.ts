...
export class MongoPostAnalysisHistoryRepository {
  ...
  async findByFingerprint(fingerprint: string): Promise<PostAnalysisHistoryItemDto | null> {
    const collection = await this.getCollection();
    const doc = await collection.findOne({ fingerprint });
    return doc ? mapPostAnalysisHistoryDocument(doc) : null;
  }

  async save(input: SavePostAnalysisHistoryInput): Promise<PostAnalysisHistoryItemDto> {
    const collection = await this.getCollection();
    const now = new Date();
...
function mapPostAnalysisHistoryDocument(doc: WithId<Document>): PostAnalysisHistoryItemDto {
  return {
    id: doc._id.toString(),
...
function normalizeAnalysis(value: unknown): PostAnalysisDto {
  ...
  return {
    ...
    cacheStatus: value.cacheStatus === "hit" || value.cacheStatus === "miss" ? value.cacheStatus : undefined,
    ...
    unknownItems: Array.isArray(value.unknownItems) ? value.unknownItems.map(normalizeUnknownItem) : [],
  };
}
...
function normalizeRow(value: unknown): PostAnalysisDto["rows"][number] {
  ...
  return {
    ...
    steamUnitPrice: value.steamUnitPrice === null || value.steamUnitPrice === undefined ? null : Number(value.steamUnitPrice),
    itemRateUnitPrice:
      value.itemRateUnitPrice === null || value.itemRateUnitPrice === undefined ? null : Number(value.itemRateUnitPrice),
    allRateTotalPrice:
      value.allRateTotalPrice === null || value.allRateTotalPrice === undefined ? null : Number(value.allRateTotalPrice),
  };
}

function normalizeUnknownItem(value: unknown): PostAnalysisDto["unknownItems"][number] {
  if (!isRecord(value)) {
