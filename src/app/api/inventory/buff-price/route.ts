...
export async function POST(request: Request) {
  try {
    ...
    const cnyToVndRate = normalizePositiveNumber(
      body.cnyToVndRate,
    ...
    const priceCny = await fetchCs2CapBuffPriceCny(
      itemId,
      apiKey,
      forceRefresh,
    );
...
async function fetchCs2CapItemId(
...
function findCs2CapLowestAsk(value: unknown): number | null {
  ...
  return typeof lowestAsk === "number" &&
    Number.isFinite(lowestAsk) &&
    lowestAsk > 0
    ? lowestAsk
    : null;
}

function findCs2CapItemId(
  value: unknown,
  marketHashName: string,
): number | null {
  ...
  for (const record of collectRecords(value)) {
    const name =
      typeof record.market_hash_name === "string"
        ? record.market_hash_name.toLowerCase()
        : "";
