...
export class PostAnalysisService {
  ...
  async analyze(
    ...
    // Luôn ưu tiên parse slang "câm" từ text nếu có để override allRate có thể sai từ Gemini
    const camRate = parseAllRate(text);
    const rawAllRate = camRate !== null ? camRate : (geminiParsedPost?.allRate ?? itemRate);
    ...
    const imageResolution =
      images.length > 0 ? resolveParsedItems(await parseInventoryImagesWithGemini(images)) : null;
    ...
    if (resolvedItems.length === 0 && unknownItems.length === 0) {
      throw new Error(images.length > 0 ? 'noCaseDetectedInPostOrImages' : 'noCaseDetectedInPost');
    }
  ...
  async analyzeWithExternalJson(
    ...
    const itemRate = chatGptJson.itemRate ?? parseItemRate(text) ?? DEFAULT_ITEM_RATE;
    const camRate = parseAllRate(text);
    const rawAllRate = camRate !== null ? camRate : (chatGptJson.allRate ?? itemRate);
    const allRate = adjustAllRate(itemRate, rawAllRate);
  ...
  private async buildPricedRows(
    ...
    const isBuffRate = (r: number) => (r >= 1.5 && r <= 5.0) || (r >= 1500 && r <= 5000);
    ...
    const rows = await mapWithConcurrency(resolvedItems, PRICE_LOOKUP_CONCURRENCY, async (item) => {
      ...
      if (useBuffPricing) {
        const buffPriceCny = await fetchBuffPriceCny(item.caseItem.marketHashName);
        if (buffPriceCny !== null) {
          itemRateUnitPrice = Math.round(buffPriceCny * itemRateVnd);
          allRateTotalPrice = Math.round(buffPriceCny * item.quantity * allRateVnd);
        } else {
          if (steamUnitPrice !== null) {
            const estimatedBuffCny = steamUnitPrice / STEAM_TO_BUFF_RATIO / CNY_TO_VND_RATE;
            itemRateUnitPrice = Math.round(estimatedBuffCny * itemRateVnd);
            allRateTotalPrice = Math.round(estimatedBuffCny * item.quantity * allRateVnd);
          }
        }
      } else {
        ...
        itemRateUnitPrice =
          steamUnitPrice === null ? null : Math.round(steamUnitPrice * normalizedItemRate);
        allRateTotalPrice =
          steamUnitPrice === null
            ? null
...
/**
 *
 * Điều chỉnh rate "all" cho mẫu slang tiếng Việt "câm".
 * Ví dụ "rate 3.7 câm .65" → rate sỉ 3.65
 *
 */
