...
export function resolveParsedItems(parsedItems: ParsedItem[]): ItemResolution {
  ...
  for (const item of parsedItems) {
    ...
    const existing = resolvedItems.find(
      (resolvedItem) =>
        resolvedItem.caseItem.marketHashName === caseItem.marketHashName,
    );
    if (existing) {
...
export function chooseItemResolution(
  ...
  return getResolvedQuantity(geminiResolution) >=
    getResolvedQuantity(localResolution)
    ? geminiResolution
    : localResolution;
}
...
export function resolveCaseItem(
  ...
  // Clean up common case descriptors to extract base name
  const caseDescriptors = [
    /\bcase\b/g,
    /\bhom\b/g,
    /\bhop\b/g,
    /\bthung\b/g,
  ...
  const isCase =
    ...
    lowerName.includes("hom") ||
    lowerName.includes("hop");
  ...
  if (
    ...
    // Normalize Souvenir (Vietnamese "Lưu niệm")
    ...
    // Remove knife bracket annotations
    formattedName = formattedName.replace(/\(\s*★\s*\)/gi, "").trim();

    // Translate knife terms from Vietnamese "Dao"
    ...
    // Capitalize words to make it presentable for Steam API
...
export function findCaseByMarketHashName(
  ...
  return (
    DEFAULT_CASES.find(
      (caseItem) => caseItem.marketHashName === marketHashName,
    ) ?? null
  );
}
