...
export function formatProgressDetail(
  ...
  if (detail.priced && detail.total)
    parts.push(t("inventoryScanner.apiErrors.itemsPriced", { priced: detail.priced, total: detail.total }));
  return parts.join(" · ");
}
...
export function translateScanProgressMessage(
  ...
  if (detail) {
    if (detail.group !== undefined) {
      options.group = detail.group === "protected" || detail.group === 16
  ...
  if (msg.includes("\u0110ang t\u1ea3i h\u00f2m \u0111\u1ed3")) {
    const groupMatch = msg.includes("th\u01b0\u1eddng") ? t("inventoryScanner.normalGroup") : t("inventoryScanner.lockedGroup");
...
export function translateAccountError(
  ...
  if (error.startsWith("familyViewInvalidParentalCookie:")) {
    const debugInfo = decodeURIComponent(error.substring("familyViewInvalidParentalCookie:debugInfo=".length));
  ...
  if (error.startsWith("geminiQuotaExceeded:")) {
    ...
    const retryText = delay 
      ...
      : (params.retryText ? ` ${params.retryText}` : "");
...
export function translateSyncMessage(
  ...
  if (msg.startsWith("syncComplete:")) {
    ...
    if (missingCount > 0 && extraCount > 0) {
      changesText = " " + t("dashboard.syncDetectedMissingAndExtra", { missing: missingCount, extra: extraCount });
  ...
  if (msg.includes("B\u1eaft \u0111\u1ea7u qu\u00e9t h\u00f2m \u0111\u1ed3:") || msg.includes("B\u1eaft \u0111\u1ea7u qu\u00e9t kho \u0111\u1ed3:")) {
