...
export function useScanState({
  ...
  const pollScanProgress = useCallback(
    async (
      ...
      while (Date.now() - startedAt < SCAN_REQUEST_TIMEOUT_MS) {
        ...
        if (!response.ok) {
          throw new Error(progress.message ?? t("inventoryScanner.apiErrors.errReadProgress", "Cannot read scan progress."));
        }
      ...
      throw new Error(t("inventoryScanner.apiErrors.errScanTimeout", "Inventory scan took too long. Please try again."));
    },
    [dispatch, t],
  );
  ...
  const doScan = useCallback(
    async (
      ...
      if (!isPartOfScanAll) {
        toastId = toast.loading(t("inventoryScanner.apiErrors.scanningInventory", "Scanning inventory..."), {
      ...
      try {
        ...
        try {
          ...
          throw new Error(t("inventoryScanner.apiErrors.errScanRequestHttp", { status: res.status, defaultValue: `Scan request failed (HTTP ${res.status}).` }));
        }
        if (!res.ok) {
          if (res.status === 401) {
            throw new Error(t("inventoryScanner.apiErrors.errLoginRequired", "Login required. Please log in to scan."));
          }
          const errorMsg = data.message ? (t(`inventoryScanner.apiErrors.${data.message}`) !== `inventoryScanner.apiErrors.${data.message}` ? t(`inventoryScanner.apiErrors.${data.message}`) : data.message) : null;
          throw new Error(errorMsg || t("inventoryScanner.apiErrors.errScanRequestGeneric", "Scan request failed."));
        }
        ...
        if (scanProgress.status === "error" || !scanProgress.result) {
          ...
          const translatedErr = rawErr ? (t(`inventoryScanner.apiErrors.${rawErr}`) !== `inventoryScanner.apiErrors.${rawErr}` ? t(`inventoryScanner.apiErrors.${rawErr}`) : rawErr) : null;
          throw new Error(
            translatedErr ??
        ...
        if (toastId) {
          ...
          toast.success(t("inventoryScanner.apiErrors.successScan", "Inventory scanned successfully!"), {
            path: "/inventory-scanner",
          });
        }
      } catch (err) {
        if (toastId) {
          ...
          if (err instanceof DOMException && err.name === "AbortError") {
            ...
            toast.error(err instanceof Error ? err.message : t("inventoryScanner.apiErrors.scanFailed", "Scan failed."), {
              path: "/inventory-scanner",
            });
          }
        }
  ...
  const scanAll = useCallback(
    async (forceRefresh = false) => {
      ...
      const toastId = toast.loading(t("inventoryScanner.apiErrors.scanningAllProgress", "Scanning all inventories in progress..."), {
        path: "/inventory-scanner",
      });
      ...
      try {
        ...
        if (signal.aborted) {
          ...
          toast.dismiss(toastId);
          toast.success(t("inventoryScanner.apiErrors.scanAllComplete", "Scan all inventories completed!"), {
            path: "/inventory-scanner",
          });
        }
      } catch (err) {
        ...
        if (err instanceof DOMException && err.name === "AbortError") {
          ...
          toast.error(t("inventoryScanner.apiErrors.scanFailedPrefix", "Scan failed: ") + (err instanceof Error ? err.message : t("common.error", "Error")), {
            path: "/inventory-scanner",
          });
        }
      } finally {
