...
export function AccessoryPreviewStrip({
  ...
  const tileClass = size === 'md' ? 'size-10 rounded' : 'size-7 rounded-md';
    ...
    <div className={cn('flex min-w-0 flex-wrap items-center gap-1.5', className)}>
      <div className="flex shrink-0 items-center gap-1">
        {visibleAccessories.map((accessory, index) => {
          const wearPercent =
            accessory.kind === 'sticker' ? formatStickerWearPercent(accessory.wear) : null;
          ...
          return (
            ...
            >
              ...
              {wearPercent ? (
                <span className="absolute inset-x-0 bottom-0 bg-black/75 px-0.5 text-center text-[8px] leading-3 font-black text-white shadow-[0_-1px_4px_rgba(0,0,0,0.5)]">
                  {wearPercent}
                </span>
              ) : null}
            </span>
      ...
      {showNames ? (
        <span
          className="max-w-[16rem] min-w-0 truncate text-[10px] font-semibold text-stone-400"
