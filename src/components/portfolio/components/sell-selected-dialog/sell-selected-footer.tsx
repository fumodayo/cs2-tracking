...
export function SellSelectedFooter({
  ...
  return (
    <div className="absolute bottom-3 left-3 right-3 z-10 flex items-center justify-between gap-4 rounded-[2px] border border-stone-800 bg-card/95 p-4 shadow-[0_8px_32px_rgba(0,0,0,0.15)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.8)] backdrop-blur-md">
      <div className="flex items-center gap-6">
        ...
        {/* Divider vertical */}
        <div className="h-8 border-l border-stone-850" />
          ...
          <div className="mt-0.5 flex items-center gap-2">
            ...
            <span
              className={`rounded-[2px] border px-1.5 py-0.5 font-mono text-[9px] font-black ${
                metrics.profitAmount >= 0
                  ? "border-emerald-500/20 bg-emerald-955/20 text-emerald-450"
                  : "border-rose-500/20 bg-rose-955/20 text-rose-450"
              }`}
            >
      ...
      <button
        ...
        className="pointer-events-auto flex h-11 cursor-pointer items-center justify-center gap-2 rounded-[2px] border border-blue-500/30 bg-gradient-to-r from-blue-700 to-blue-600 px-6 shadow-[0_4px_20px_rgba(37,99,235,0.15)] transition-all duration-300 hover:from-blue-600 hover:to-blue-500 hover:shadow-[0_4px_25px_rgba(37,99,235,0.3)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 text-white"
      >
        {bulkLoading ? (
          <Loader2 className="size-4 animate-spin text-white" />
        ) : (
          <>
            <ShoppingBag className="size-3.5 text-white fill-white/20" />
