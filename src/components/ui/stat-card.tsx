...
type StatCardProps = {
  title?: string;
  label?: string; // Bí danh cho title
  value: string;
  numericValue?: number;
...
const toneClasses = {
  ...
  violet: 'border-amber-500/20 bg-amber-500/5 text-stone-50', // Ánh xạ violet sang amber.
};

const iconContainerClasses = {
  neutral:
    'bg-surface-muted text-muted-foreground border border-border dark:bg-stone-800/50 dark:text-stone-400 dark:border-stone-800',
...
export function StatCard({
  ...
  // Resolve tone của card (cho phép fallback từ tone hoặc variant)
  const resolvedTone = tone || variant || 'neutral';

  // Chuẩn hóa tone để map style
  const mappedTone = resolvedTone === 'violet' || resolvedTone === 'amber' ? 'amber' : resolvedTone;

  const displayTitle = title ?? label ?? '';

  // Tự resolve icon theo mappedTone nếu không truyền icon
  const getIcon = () => {
    if (icon) return icon;
    switch (mappedTone) {
      case 'blue':
  ...
  return (
    <Card
      className={cn(
        'h-full border p-4 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)]',
        customClass
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {detail && <div className="text-muted-foreground mt-2 text-sm">{detail}</div>}
        </div>
        <div
          className={cn(
